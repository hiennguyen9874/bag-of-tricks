...
from .datasets import ImageDataset, DatasetCache
...
class DataManger(BaseDataManger):
    def __init__(self, config, phase="train"):
        ...
        self.training_set = ImageDataset(
        ...
        self.test_set = {
            "query": ImageDataset(
                self.datasource.get_data("query"), transform=self.transform_test
            ),
            "gallery": ImageDataset(
                self.datasource.get_data("gallery"), transform=self.transform_test
            ),
        }
        ...
        if phase == "train":
            ...
            ).split(rate=1 - config["validation_split"])
