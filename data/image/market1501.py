...
class Market1501(object):
    ...
    def __init__(
        self, root_dir="datasets", download=True, extract=True, re_label_on_train=True
    ):
        ...
        ) = self.process_dir(self.train_dir, relabel=re_label_on_train)
        ...
        ) = self.process_dir(self.query_dir, relabel=False)
        ...
        ) = self.process_dir(self.gallery_dir, relabel=False)
    ...
    def _extract(self):
        ...
        try:
            ...
            for member in tqdm(
                iterable=zip_ref.infolist(), total=len(zip_ref.infolist())
            ):
    ...
    def _exists(self, extract_dir):
        if (
            os.path.exists(os.path.join(extract_dir, "bounding_box_train"))
...
if __name__ == "__main__":
    market1501 = Market1501(
        root_dir="/home/hien/Documents/datasets", download=True, extract=True
    )
    print(
        "Train: len: {}, num_class: {}, num_camera: {}".format(
            len(market1501.train),
            market1501.get_num_classes("train"),
            market1501.get_num_camera("train"),
        )
    )
    print(
        "Query: len: {}, num_class: {}, num_camera: {}".format(
            len(market1501.query),
            market1501.get_num_classes("query"),
            market1501.get_num_camera("query"),
        )
    )
    print(
        "Gallery: len: {}, num_class: {}, num_camera: {}".format(
            len(market1501.gallery),
            market1501.get_num_classes("gallery"),
            market1501.get_num_camera("gallery"),
        )
    )
